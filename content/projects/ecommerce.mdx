---
title: "E-commerce Backend API"
date: "2025-08-18"
description: "Modular and production-ready API built with NestJS, TypeORM, and PostgreSQL — featuring JWT authentication, role-based access, pagination, and Cloudinary image management."
image: "/ecommerce-api.png"
published: true
slug: "ecommerce"
technologies: ["NestJS", "TypeScript", "TypeORM", "PostgreSQL", "JWT", "Swagger", "Cloudinary"]
links:
  live: "/projects/ecommerce/api-docs"
  github: "https://github.com/NicoleMCardenas/PM4BE-NicoleMCardenas"
---

<div className="mt-10">
  <img
    src="/ecommerce-api.png"
    alt="Swagger UI — E-commerce Backend API"
    className="mx-auto w-full max-w-4xl rounded-2xl border border-zinc-800 shadow-2xl"
  />
  <p className="mt-3 text-center text-sm text-zinc-500">
    Swagger UI — E-commerce Backend API
  </p>

  <div className="mt-6 flex justify-center">
  <a
    href="/projects/ecommerce/api-docs"
    className="inline-flex items-center gap-2 rounded-lg border border-zinc-700/60 bg-zinc-900/20 px-4 py-2 text-sm font-medium text-zinc-300 transition-all duration-200 hover:bg-zinc-800/40 hover:text-white hover:border-zinc-500 backdrop-blur-sm"
  >
    View API Docs
    <span aria-hidden="true" className="text-zinc-400 group-hover:text-white">
      →
    </span>
  </a>
</div>
</div>

---

**Backend-first, scalable, and production-ready.**  
This RESTful API was designed for e-commerce platforms requiring modular architecture, strict validations, and seamless integrations.  
Built with **NestJS**, **TypeORM**, and **PostgreSQL**, it handles authentication (JWT + role-based guards), order management, and media uploads through **Cloudinary**.

---

### 🚀 Key Features
- **Secure Authentication** with JWT and role-based access control (Admin, Seller, Customer).  
- **Modular Architecture** using NestJS and TypeORM entities.  
- **Pagination & Filtering** for product and order listings.  
- **Image Uploads** handled via Cloudinary integration.  
- **OpenAPI/Swagger Documentation** auto-generated and hosted directly in the project.

---

### 🧠 Tech Stack
NestJS • TypeScript • TypeORM • PostgreSQL • Swagger • Cloudinary • JWT

---

### 💡 Extra Context
This project was developed as part of a full-stack training program to demonstrate backend design best practices — focusing on maintainability, clean code, and developer experience.
